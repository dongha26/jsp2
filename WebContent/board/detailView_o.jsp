...
<main>
			...
			<li>
				...
				<ul style="display: flex;">
					...
					<!-- sessionScope.user는 session에 저장된 애트리뷰트 user를 가져옵니다
						sessionScope을 생략할 수 있지만 request,application등 다른 영역에 중복된 이름이 있을때는 
						고려하여 명시적으로 지정합니다  -->
					<c:if test="${sessionScope.user != null }">
						<button type="submit">저장</button>
						<button type="reset" onclick="reset_content()">취소</button>
					</c:if>
					<c:if test="${sessionScope.user ==null }">
	...
	function login(){
		...
		//다시 지금 페이지로 돌아오기 위해 필요한 값 mref,page 요소값을 전달합니다.(textarea는 불필요.)
